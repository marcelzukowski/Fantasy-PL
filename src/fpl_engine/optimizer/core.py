...
class Optimizer:
 ...
 def __init__(self,rules:OptimizerRules):self.rules=rules
 def recommend(self,state:SquadState,projections:Mapping[str,PlayerProjection],pool:Iterable[SquadPlayer]=(),*,max_transfers=2,chip:str|None=None)->Recommendation:
  ...
  candidates=list(pool);owned={p.player_id for p in state.players}
  for count in range(1,0 if chip in {'wildcard','free_hit'} else min(max_transfers,int(self.rules.value('transfers','normal_gameweek','maximum_regular_transfers_per_gameweek')))+1):
   for outs in combinations(state.players,count):
    for ins in combinations([p for p in candidates if p.player_id not in owned],count):
     new=[p for p in state.players if p not in outs]+list(ins);temp=SquadState(tuple(new),0,state.free_transfers,state.chips,state.current_gameweek,state.season,state.rule_version,state.prediction_timestamp)
     try:validate_squad(temp,self.rules)
     except OptimizerError:continue
     bank=state.bank+sum(p.selling_price if p.selling_price is not None else selling_price(p.purchase_price,p.current_price,self.rules) for p in outs)-sum(p.current_price for p in ins)
     if bank<0 or any(p.player_id not in projections for p in ins):continue
     gross=sum(projections[p.player_id].weighted_ev_next_6 for p in ins)-sum(projections[p.player_id].weighted_ev_next_6 for p in outs);paid=max(0,count-state.free_transfers);hit=paid*int(self.rules.value('transfers','normal_gameweek','points_cost_per_transfer_above_allowance'))
     plans.append(TransferPlan(tuple(sorted(p.player_id for p in outs)),tuple(sorted(p.player_id for p in ins)),gross,hit,gross-hit,bank,min(count,state.free_transfers),next_free_transfers(state.free_transfers,count,self.rules),False))
  plans.sort(key=lambda p:(-p.net_gain,p.transfers_out,p.transfers_in));best=plans[0]
  ...
  lineup=optimize_lineup(working,projections,self.rules,triple_captain=chip=='triple_captain',bench_boost=chip=='bench_boost')
 ...
 def _unlimited_squad(self,state,projections,pool):
  candidates=tuple({p.player_id:p for p in state.players+pool}.values());n=len(candidates);owned={p.player_id:p for p in state.players};values=np.array([projections[p.player_id].weighted_ev_next_6 for p in candidates]);cost=np.array([(owned[p.player_id].selling_price if owned[p.player_id].selling_price is not None else selling_price(owned[p.player_id].purchase_price,owned[p.player_id].current_price,self.rules)) if p.player_id in owned else p.current_price for p in candidates]);funds=state.bank+sum(selling_price(p.purchase_price,p.current_price,self.rules) if p.selling_price is None else p.selling_price for p in state.players);constraints=[LinearConstraint(np.ones((1,n)),15,15),LinearConstraint(cost.reshape(1,-1),-np.inf,funds)]
