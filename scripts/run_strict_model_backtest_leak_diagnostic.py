...
def run(
    ...
    simulator = FixtureSimulator(historical_scoring_engine, SimulationConfig(
    ...
    for gw in sorted(snapshots):
        ...
        history_matches = [row for row in match_rows if row.known_at <= at and row.kickoff < at]
        ...
        for state in current_states.itertuples():
            ...
            _target_in_history = [
                ...
                if row.fixture_id == state.canonical_fixture_id
            ]

            if _target_in_history:
                print()
                print("=" * 100)
                print("STRICT TEAM-STRENGTH LEAK DIAGNOSTIC")
                print("=" * 100)

                print("GW:", gw)
                print("prediction_at:", at)
                print("provider_fixture_id:", state.provider_fixture_id)
                print("canonical_fixture_id:", state.canonical_fixture_id)
                print("scheduled_gameweek:", state.scheduled_gameweek)
                print("scheduled_kickoff:", state.scheduled_kickoff)
                print("information_known_at:", state.information_known_at)
                ...
                print()
                print(
                    "history_matches total:",
                    len(history_matches),
                )

                raise SystemExit(
                    "DIAGNOSTIC_STOP_TARGET_ALREADY_IN_HISTORY"
                )
    ...
    projection_metadata = ExperimentMetadata(
        ...
        model_versions=("team_strength_v1", "minutes_hurdle_v1",
                        ...
                        "fixture_simulator_v1", "projection_builder_v1"),
    ...
    convergence_configuration = {
        ...
        "model_versions": ["coherent_assists_v2" if use_assist_v2 else "events_v1",
                           "fixture_simulator_v1", "projection_builder_v1", "optimizer_v2"],
    ...
    report = {
        "report_version": 3 if (advanced_challenger or talent_v2_only) else 2, "mode": "STRICT", "season": season,
        "candidate_mode": (
            "ADV_CHALLENGER" if advanced_challenger
            else "TALENT_V2_ONLY" if talent_v2_only
            else "SIM_V1_512_CONTROL"
        ),
    ...
    output = root / "data" / "processed" / "backtests" / "strict" / season / "simulator_ab" / "v1_512"
...
if __name__ == "__main__":
    ...
    print(*(str(path) for path in run(
